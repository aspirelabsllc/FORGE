...
/**
 * An invitation may be viewed by the person who sent it, the person it was sent
 * to, or an existing member of the target project. Anyone else has no business
 * seeing another user's invitation (email, role, inviter identity, and - for the
 * raw `get` procedure - the accept token).
 */
async function verifyInvitationViewAccess(
    ctx: { db: Parameters<typeof verifyProjectAccess>[0]; user: { id: string; email: string } },
    invitation: { projectId: string; inviterId: string | null; inviteeEmail: string },
): Promise<void> {
    if (ctx.user.email === invitation.inviteeEmail || ctx.user.id === invitation.inviterId) {
        return;
    }
    try {
        await verifyProjectAccess(ctx.db, ctx.user.id, invitation.projectId);
    } catch {
        throw new TRPCError({
            code: 'NOT_FOUND',
...
export const invitationRouter = createTRPCRouter({
    get: protectedProcedure.input(z.object({ id: z.string() })).query(async ({ ctx, input }) => {
        ...
        await verifyInvitationViewAccess(ctx, invitation);
    ...
    create: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            ...
            // Only existing project members may invite others - otherwise anyone could
            // invite themselves (with any role, including OWNER) to an arbitrary
            // project and then self-accept via `accept`.
            await verifyProjectAccess(ctx.db, ctx.user.id, input.projectId);
