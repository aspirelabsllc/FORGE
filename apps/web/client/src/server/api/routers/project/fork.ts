...
export const fork = protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
        ...
        // Forking is allowed either for projects the user has access to (cloning your
        // own project), or for projects explicitly marked as public templates (the
        // "Use Template" flow). Anything else would let any authenticated user copy
        // another user's private project by guessing/knowing its id.
        const isTemplate = sourceProject.tags?.includes(Tags.TEMPLATE) ?? false;
        if (!isTemplate) {
