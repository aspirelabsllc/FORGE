...
import { createForgeProposeStream } from '@onlook/forge';
import { ChatType, type ChatMessage, type ChatMetadata } from '@onlook/models';
import { type NextRequest } from 'next/server';
import { v4 as uuidv4 } from 'uuid';
import { checkMessageLimit, decrementUsage, errorHandler, getBrandKitForProject, getSupabaseUser, incrementUsage } from './helpers';
...
export const streamResponse = async (req: NextRequest, userId: string) => {
    ...
    try {
        ...
        const stream = chatType === ChatType.FORGE_PROPOSE
            ? createForgeProposeStream({
                brandKit: await getBrandKitForProject(req, projectId),
                conversationId,
                projectId,
                userId,
                traceId,
