...
import { flattenColorTokens } from '@onlook/brand-schema';
...
export const ColorPickerContent: React.FC<ColorPickerProps> = ({
    ...
    // When the project has a linked BrandKit with color tokens, constrain
    // this picker to on-brand values only - a token picker, not a raw color
    // wheel, per the differentiator called out for the GUI editing MVP.
    const { data: brandKit } = api.brandKit.getForProject.useQuery({
        projectId: editorEngine.projectId,
    });
    const brandColorSwatches = flattenColorTokens(brandKit?.identity.tokens.color);
    const brandTailwindColors: TailwindColor[] = brandColorSwatches.map((swatch) => ({
        name: swatch.name,
        originalKey: swatch.name,
        lightColor: swatch.value,
    }));
    const isConstrainedToBrandKit = brandTailwindColors.length > 0;

    useEffect(() => {
        if (isConstrainedToBrandKit && activeTab !== TabValue.BRAND) {
            setActiveTab(TabValue.BRAND);
        }
    }, [isConstrainedToBrandKit, activeTab]);
    ...
    const filteredBrandKitColors = brandTailwindColors.filter((swatch) =>
    ...
    return (
        <div className="flex flex-col justify-between items-center">
            ...
            >
                {!isCreatingNewColor && (
                    <TabsList className="bg-transparent px-2 m-0 gap-2 justify-between w-full">
                        <div className="flex gap-1">
                            ...
                            {!hideGradient && !isConstrainedToBrandKit && (
                                <TabsTrigger
                ...
                {!isCreatingNewColor && (
                    <TabsContent value={TabValue.BRAND} className="p-0 m-0 text-xs">
                        ...
                        <div className="flex flex-col gap-1 overflow-y-auto max-h-96 px-2 mt-2">
                            {filteredBrandKitColors.length > 0 && (
                                <ColorGroup
                                    name="Brand Kit"
                                    colors={filteredBrandKitColors}
                ...
                {(isCreatingNewColor || !isConstrainedToBrandKit) && (
