...
import { extractTouchedTargets } from '@onlook/attribution';
...
export class ActionManager {
    ...
    async run(action: Action) {
        ...
        this.recordOwnership(action);
    }

    /**
     * Fire-and-forget: records every oid this GUI action touched as
     * user-owned, per branch. Never blocks or fails the actual edit - the
     * ledger is a best-effort record, not a gate on this path (the gate is
     * Forge's regeneration checking against it, see @onlook/attribution's
     * checkForConflicts).
     */
    private recordOwnership(action: Action) {
        const touched = extractTouchedTargets(action);
        if (touched.length === 0) {
            return;
        }
        const byBranch = new Map<string, string[]>();
        for (const { oid, branchId } of touched) {
            byBranch.set(branchId, [...(byBranch.get(branchId) ?? []), oid]);
        }
        for (const [branchId, oids] of byBranch) {
            api.attribution.recordEdits.mutate({ branchId, oids, editor: 'user' }).catch((error) => {
                console.error('Failed to record ownership for GUI edit', error);
            });
        }
    }
