...
import type { GitAuthor } from '@onlook/attribution';
...
export class GitManager {
    ...
    /**
     * Get the diff of uncommitted changes against HEAD, optionally scoped to
     * specific file paths - used to gather Aegis's `codeDiff` input after a
     * Forge PROPOSE turn's `submit_for_critique` call.
     */
    async getDiff(filePaths?: string[]): Promise<string> {
        const pathsArg = filePaths?.length
            ? ' -- ' + filePaths.map((p) => escapeShellString(p)).join(' ')
            : '';
        const result = await this.runCommand(`git --no-pager diff HEAD${pathsArg}`);
     ...
     * `author`, when given, is passed as an inline `-c user.name=/-c user.email=`
     * override rather than mutating the repo's persistent git config - so a
     * Forge-authored commit and a concurrent user-authored one can't race on
     * shared config state. See @onlook/attribution's FORGE_GIT_AUTHOR/
     * buildUserGitAuthor - this is the "nearly free" commit-level attribution
     * half of the ownership model (the node-level ledger is the other half).
     */
    async commit(message: string, author?: GitAuthor): Promise<GitCommandResult> {
        const sanitizedMessage = sanitizeCommitMessage(message);
        const escapedMessage = prepareCommitMessage(sanitizedMessage);
        const authorFlags = author
            ? `-c user.name=${escapeShellString(author.name)} -c user.email=${escapeShellString(author.email)} `
            : '';
        const result = await this.runCommand(
            `git ${authorFlags}commit --allow-empty --no-verify -m ${escapedMessage}`,
        );
        if (result.success) {
            await this.listCommits();
        }
    ...
    async createCommit(message = 'New Onlook backup', author?: GitAuthor): Promise<GitCommandResult> {
        ...
        // Create the commit
        return await this.commit(message, author);
    }
