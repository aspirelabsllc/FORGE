...
const SupabaseStatusSchema = z
    .object({
        // Legacy JWT keys are deprecated by Supabase; kept optional so setup keeps
        // working whether or not the local CLI still emits them.
...
export const generateBackendEnvContent = (
    ...
    for (const item of config) {
        ...
        if (item.key === 'NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY') {
