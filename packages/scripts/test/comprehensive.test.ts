...
SUPABASE_DATABASE_URL=postgresql://postgres:postgres@127.0.0.1:54322/postgres`;
            ...
            expect(lines[1]).toBe(`NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY=${mockKeys.publishableKey}`);
...
SUPABASE_DATABASE_URL=postgresql://postgres:postgres@127.0.0.1:54322/postgres`;
            ...
            expect(lines[1]).toBe(`SUPABASE_SECRET_KEY=${mockKeys.secretKey}`);
            expect(lines[2]).toBe(
                'SUPABASE_DATABASE_URL=postgresql://postgres:postgres@127.0.0.1:54322/postgres',
            );
...
ANTHROPIC_API_KEY=test_anthropic_key_placeholder_def
...
MAX_UPLOAD_SIZE=52428800`;
            ...
            expect(parsedVars.has('ANTHROPIC_API_KEY')).toBe(true);
            expect(parsedVars.get('ANTHROPIC_API_KEY')?.value).toBe(
                'test_anthropic_key_placeholder_def',
            );
...
SUPABASE_DATABASE_URL=postgresql://postgres:postgres@127.0.0.1:54322/postgres`;
        ...
        it('should build complete env file content with mixed variables using actual function', () => {
            ...
            envVars.set('ANTHROPIC_API_KEY', {
                key: 'ANTHROPIC_API_KEY',
                value: 'test_anthropic_key_placeholder_123',
            });
